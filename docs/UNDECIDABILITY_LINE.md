  ...
  **M_tr 本體下 session 起手點**：(4a) 定 `encCfg : Cfg' → M_tr.Cfg`（控制用 M18 trLabelEquivFin
  ...
  - **4b 路線決策（使用者定）= 接 mathlib TM 化約鏈（route β）**。**關鍵發現**：mathlib
    ...
    - **4b-4 = TM0(Bool) → 我們 `BitTM` 橋**（route β 唯一需動腦接口），拆 4 子鑽：
      ...
      - **剩 4b-4 子鑽**：(4b-4c) **定 `M_tr : BitTM` + 一步模擬** `M_tr.step (enc c) = enc (TM0.step c)`
        （讀 `t 0` → `decCtrl` 狀態 → `TM0.tr q head` → 動作，組帶橋 M25 `tapeStep_nth` + 狀態編碼 M26；
        `Option`→total [M16 totalize]）→ (4b-4d) 停機橋 `M_tr.step^[k]` ⟺ `TM0.step^[k]`。
