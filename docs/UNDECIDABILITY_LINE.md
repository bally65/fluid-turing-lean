...
- **子塊 4 動力學組裝 ✅（M20）+ M_tr 本體 = 唯一剩餘缺口**。
  ...
  **M_tr 本體下 session 起手點**：(4a) 定 `encCfg : Cfg' → M_tr.Cfg`（控制用 M18 trLabelEquivFin
  ...
  - **4b 路線決策（使用者定）= 接 mathlib TM 化約鏈（route β）**。**關鍵發現**：mathlib
    ...
    - **剩 route β 鑽**：(4b-2) `TM1to1`（Γ'→Bool 字母，由 `Respects` 導出 eval-dom）→
      (4b-3) `TM1to0`（→ TM0 Bool，`tr_eval`）→ (4b-4) **TM0(Bool) → 我們 `BitTM` 橋**
      （`Λ→Fin m` 用 `tr_supports` 的 Finset 壓 [M18/M21 手法]、`Tape Bool ↔ ℤ→Bool` 表示轉換、
      `Option`-step → total [M16 totalize]）→ (4b-5) 接 M16 兌現 `hmachine` + `hHclosed` → M20 收**無條件**。
