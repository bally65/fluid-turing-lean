    ...
    - **4b-5 接 M16 兌現 → M20 收無條件（拆 2）**：
      ...
      - **4b-5b 拆 2**：
        ...
        - **4b-5b-B 化約鏈兌現 `hcorr`（拆多段，比預估大）**：
          ...
          - **剩 B1-B4（誠實：實為大型 mathlib-internals 組裝、非「最後一哩」）**：mathlib **不預組合**
            化約鏈，且 `TM2to1.tr_eval_dom` 用泛型 `TM2.init`、M16/M30 用具體 `PartrecToTM2.init`
            ——**真障礙**：需組態層 `tr_respects` + **手建 `TrCfg (init cu ...) c2`**（B1，mathlib
            `trCfg_init` 只給泛型 init），再 threading TM1to1（B2，`exists_enc_dec` 的 Classical 編碼）
            /TM1to0（B3）組態翻譯，組 `M_univ` + 實例（B4）。**機械真確、mathlib 有全零件、非 paper-blocked、
            但實為多 session 純接線工程、零概念內容。**
