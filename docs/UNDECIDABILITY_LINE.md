    ...
    - **4b-5 接 M16 兌現 → M20 收無條件（拆 2）**：
      ...
      - **4b-5b 拆 2**：
        ...
        - **4b-5b-B 化約鏈兌現 `hcorr`（拆多段，比預估大）**：
          ...
          - **剩 B4**（最後一磚）：supports threading（`hSupp` 由 `tr_supports` 鏈組合 + **init label ∈
            S_univ** 3 段穿線：`trNormal cu halt ∈ codeSupp` [M18] → `normal` ∈ trSupp → ... → stmts）+
            組 `M_univ`/`S_univ` + 接 M30 的 cu 橋 + `apply mtr_flow`（M29）→ `hcorr` 兌現 →
            `fluid_blowup_undecidable` **無條件**。
