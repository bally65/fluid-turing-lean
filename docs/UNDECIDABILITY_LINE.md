  ...
  - **4b 路線決策（使用者定）= 接 mathlib TM 化約鏈（route β）**。**關鍵發現**：mathlib
    ...
    - **4b-5 接 M16 兌現 → M20 收無條件（拆 2）**：
      ...
      - **4b-5b 拆 2**：
        ...
        - **剩 4b-5b-B** 化約鏈兌現 `hcorr`：造 reduced 通用 TM0-Bool 機器 `M_univ`（3 段 `tr` 合成 +
          `exists_enc_dec` 編碼）+ 3 段 eval-dom 組合（thread init 組態翻譯）+ M16
          `tm2_halts_iff_code_eval_dom` → `eval(TM0.step M_univ)(c0 code).Dom ⟺ code.eval.Dom`。
          純 mathlib 接線、無條件收尾。
