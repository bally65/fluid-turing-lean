    ...
    - **4b-5 接 M16 兌現 → M20 收無條件（拆 2）**：
      ...
      - **4b-5b 拆 2**：
        ...
        - **4b-5b-B 化約鏈兌現 `hcorr`（拆多段，比預估大）**：
          ...
          - **剩 B2-B4**（`exists_enc_dec` Classical 編碼 + `M_univ` 組裝，一體）：B2 TM1to1（`tm1to1_halts_iff`
            + `trCfg` init 翻譯 rfl）→ B3 TM1to0（`tm1to0_halts_iff`）→ B4 組 `M_univ` + 接 M30/M31 鏈兌現
            `hcorr` → `mtr_flow` 變無條件。**機械真確、非 paper-blocked、純接線。**
