  ...
  **M_tr 本體下 session 起手點**：(4a) 定 `encCfg : Cfg' → M_tr.Cfg`（控制用 M18 trLabelEquivFin
  ...
  - **4b 路線決策（使用者定）= 接 mathlib TM 化約鏈（route β）**。**關鍵發現**：mathlib
    ...
    - **4b-5 接 M16 兌現 → M20 收無條件（拆 2）**：
      ...
      - **剩 4b-5b** 化約鏈組裝：接 M16 `tm2_halts_iff_code_eval_dom` + M22/23/24 三段化約橋（thread
        init 組態翻譯），得「M_tr（通用機器）迭代到 `encHalt` ⟺ `code.eval` 停機」= 兌現 `hmachine`，
        實例化 `bitTM_bennett_blowup_undecidable` → **收無條件**。
