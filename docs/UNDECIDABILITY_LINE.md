...
- **子塊 4 動力學組裝 ✅（M20）+ M_tr 本體 = 唯一剩餘缺口**。
  ...
  **M_tr 本體下 session 起手點**：(4a) 定 `encCfg : Cfg' → M_tr.Cfg`（控制用 M18 trLabelEquivFin
  ...
  - **4a ✅（M21）** `encCfg : Cfg' → (Option Λ' × Option Γ' × (ℤ→Bool))`（位帶 = M19 encStacks、
    ...
    位元向量 + 接動力學 = 4b。
  - **4b（下一塊）** M_tr : BitTM 的**定義**：`m` = ctrlLabels/var/phase/offset 的位元數；
    `next/write/move` = 解釋器迴圈（讀控制 label → 查 `tr l` 的 Stmt 樹 → 逐堆疊 op 走位+編輯）。
    先做**純控制建構子**（goto/load：不碰帶、只換 label）的單微步引理暖身，再上走位型（push/pop/peek）。
