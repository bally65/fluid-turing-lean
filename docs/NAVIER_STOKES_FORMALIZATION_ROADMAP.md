...
common-carrier time continuity；C183–C247 已完成 summable `H⁻³` weight、complete coefficient
...
接到 C223，證 sufficiently late windows 上完整 velocity/viscous consumer穩定。
在 `ν>0` 的 actual-family theorem 中，前述速度、乘積、C207 pairings與 weak identity沿用同一
`U,φ`。**C247 checkpoint 已完成：contact code checkpoint `612e758` 與 receipt `92e2a97`
已推上 `origin/main`；full build、zero-sorry、5126-declaration axiom gate與 244/244 consistency
均通過，C220–C247 沒有新增 warning。全庫仍有 C049–C129 的 161 個 legacy warning，所以
`integrate.sh` 只在 warning-baseline 規則上非零。**checkpoint 後的最低成本數學 gate，是把
C247 的 velocity-window stability複製到 C224 product slot，再合併 initial slot與 C225 complete
consumer。其後才可定義真正 intrinsic/complete topology與 physical representation，並在正確 image
...
| 距離層 | 現在位置 | 還缺什麼 | 誠實判定 |
| --- | --- | --- | --- |
| 固定 cutoff Galerkin / compactness gate | C153–C247 已完成 fixed-cutoff global core、共同 field reconstruction 與 uniform estimates；在 `ν>0` 時，一條 `StrictMono` cutoff subsequence 在每個 `[t₀,t₀+m]` 強收斂，local quotient limits 與所有 coordinate products 沿同一 subsequence收斂；fixed/limiting finite-Fourier identities、canonical LF zero-final tests、realizable residual span、finite-family identities、ambient/residual-kernel obstruction、complete algebraic tensor consumer/image、fixed/global faithful quotients，以及 sufficiently-late velocity-window stability亦完成 | product/complete-consumer window stability、completed intrinsic physical-test topology、正確 density/closure theorem、physical representation、pointwise initial trace與 standard PDE identification | **whole ambient consumer 與 whole residual kernel 都已證過大；尚非一般 test-space weak solution** |
| 已知週期 3D NS 理論 | Fourier/Parseval/Galerkin、energy/gradient bounds、negative-space time control、compatible local strong limits、coordinate/product carriers、finite-Fourier cutoff-limit identities、canonical LF time tests、finite smooth-test weak identities、faithful algebraic consumer quotients與 velocity-window stability已有 | intrinsic/completed Sobolev、完整 cross-window test carrier、canonical joint-product carrier或足夠 coordinate representation、global representative、weak gradient、adequate smooth-test weak formulation、pointwise initial trace、energy inequality、Leray–Hopf、local strong、weak–strong uniqueness、條件正則性 | **仍很遠；C220–C247 建立了正確 algebraic image/quotient與一個穩定槽，但沒有 physical-test density 或 weak solution** |
...
| Clay 未解核心 | 尚無 theorem | 對任意大光滑 divergence-free 3D 初值證永遠光滑，或構造合格 finite-time singularity | **距離未知、不可估工期；C247 不改變此判定**；需要新數學，不是多蓋幾磚必然到達 |
...
compactness/subsequence conclusions 下，C178–C247 已把這些
...
與 whole residual kernel 都過大的反例；C221–C247 再接上 canonical LF zero-final time tests、
finite-family weak identities、complete algebraic tensor consumer、faithful fixed/global quotients、
all-window image factorization、tail vanishing、zero-extension integral與 velocity-window stability。
**C247 contact checkpoint `612e758`／`92e2a97` 已推送，build／axiom／consistency receipts如上。**
其後先補 product/complete consumer window stability，再處理 intrinsic physical-test image、正確 density/closure、足夠的
...
**在 `ν>0` 的 actual compactness/subsequence conclusions 下，C178–C247 已完成 NS050/NS070
...
6. **C247 GitHub checkpoint（使用者指定的週期保存）：**C236–C247 已完成 targeted build、
...
**C220–C247 後續已把上述負向 gate接到 canonical LF time-test carrier、literal
realizable provenance、finite-family identities、complete algebraic tensor consumer、必要
image constraints、faithful quotients與第一個 velocity-window stability theorem；contact
checkpoint `612e758`／`92e2a97` 已推送。**
...
| NS040 | `Galerkin` | **C153–C247 已完成 fixed-cutoff core、compatible local compactness/products、signed spacetime identity、同一 subsequence limit、canonical LF zero-final tests、finite-family identities、consumer obstruction、complete algebraic tensor consumer、faithful fixed/global quotient與 velocity-window stability：**同一 `U,phi` 支援所有 integer windows與 algebraic tensors；C247 首次消除 sufficiently-late velocity slot的 window presentation依賴 | fixed-cutoff、compactness、products與 algebraic tensor-test tickets complete；下一 gate 是 product/complete-consumer window stability、intrinsic physical-test image與正確 density，不是 whole ambient/residual kernel density |
| NS050 | `TorusSobolev` | **finite-field/consumer 第一層由 C178–C247 完成：**common synthesis/derivatives、homogeneous gradient bound、negative carrier、compactness、iterated/projective velocity/product carriers、finite-Fourier identities、transverse/reality carriers、canonical LF time carrier、complete algebraic tensor consumer、faithful quotients與一個穩定 consumer slot；仍欠 complete cross-window carrier、canonical joint-vector carrier equivalence、completed/intrinsic Sobolev package、intrinsic test topology與 global representative | 不把 algebraic tensor或 consumer bounded-convergence topology冒充 completed physical-test topology，也不把 image constraints說成 image equality或 intrinsic-test density |
...
- C153–C247 已把 Leray/Stokes/convection 組成有限維 ODE，證任意 initial time 的 unique
  ...
  carrier 上 forward time-continuous；C183–C247 另完成 common coefficient `H⁻³` carrier、
  ...
  envelope、endpoint／FTC與 actual/stored-family fixed-cutoff spacetime weak identity；C212–C247 已續作
  ...
  factorization、compact-support tail control、zero-extension integration與 velocity-window stability。
...
15. **C220–C247 canonical test/tensor quotient與第一個 window-stable slot已完成並 checkpoint：**whole residual kernel 過大的
...
16. **續補完整 cross-window bridge：**C247 已完成 velocity/viscous slot；下一步先用 C205 product
   restriction證 C224 九項 product slot穩定，再合併 initial slot與 C225 complete consumer。其後才選擇真正
