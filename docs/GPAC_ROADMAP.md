...
## 2026-07-11 更新⑩：G5 vs G4e 設計 scope 裁決 + M55 G4e 無限帶橋落地

**設計 scope workflow（wf_cf012cb9、6 researcher + synth）**裁決往下走哪條：

**★G5 全檔 tube = 已證 WALL（high conf、Lean-certified）★**：3 researcher 全 WALL。sigmaRL 在
plateau-exact tube 上單步 Jacobian = diag(0, 1/K, K)（狀態 tbl2D 增益 0、寫入 push 1/K、**讀方向 pop
`K·y−sfloor(K·y)` 增益恰 K**——sfloor plateau 只殺「符號減法」增益、不殺 `K·y` 拉伸；**certified
`pop_gain_K` 編譯過**）。單步譜半徑 ρ=K>1 ⟹ **任何加權範數皆無法收縮**（ρ(A)≤‖A‖）、`bounded_orbit`
的 Λ<1 不可達。放大的誤差住**分數**帶暫存器（encTape 值 = spacing K^{−ℓ}→0 的無分離集）、**無**固定
解析度 C^∞ 量化器（sround/snapExact 只認整格）可清。連續流 robustness 的正面目標**死**於此牆（除非
per-symbol re-round 全新架構 = paper-level 多月、且分數暫存器清理本身開放）。**本磚族不再攻 G5 正面。**

**★G4e = MULTI_ROUND achievable（已建 M55）★**：`Turing.ListBlank ℕ` 就是現成 eventually-blank 半帶
（blank=0）；`encInf` 經 `liftOn` 昇（良定義=blank-append 不變、純代數零實分析、researcher 全驗編譯）。
**M55 落地**：`encInf`（+ `encInf_head_tail` 無條件 head/tail 分解 = bothNonempty 消失根源）、
`read_plateau_B`（無條件頭可讀）、`GCfgB`/`gEncB`/`gStepRLB`（全函數無 fallback）/`wfCfgB`/`gStepRLB_wf`、
**`sigmaRL_exact_of_wfB`（CRUX：σ 無 plateau 假設無 bothNonempty 字面精確）**、
**`sigmaRL_iterate_exactB`（HEADLINE：N 步無 hne）**。**M50 的 `bothNonempty` 缺口永久填平、左端邊界
（有限 List 內稟）消失。** 56 模組零 sorry 標準三公理。

**★誠實★**：G4e 去的是**帶邊界假設**、非加通用性。仍玩具（gStepRLB module-local、未接 mathlib TM/
...
## 2026-07-12 更新⑭：★★G6 封頂 — M59「顯式 C^∞ 映射軌道可達性不可判定」（GPAC 線頭號結果）★★

**承 M57(gStepRL3=BitTM 一步)+M58(sigmaRL3=gStepRL3 步對步 C^∞)+M28(Mtr_halts_iff 無條件)+M33
(通用碼 block 直接複用)**。M59 把整條 GPAC 鏈焊到 mathlib 停機問題,得**第一個關於「顯式光滑映射」
的不可判定結果**(全機器背書、零 sorry、標準三公理)。

**鏈條**: mathlib halting_problem → (M33 block) TM0 停機 → (Mtr_halts_iff M28) (Mtr M S).step^[k+1]
到 encHalt → (bitStep_iterate 本磚) bitStepTape^[k] 到 vhalt → (gStepRL3_iterate_tape 本磚) →
(gStepRL3_simulates_BitTM M57) → (reach_at_k 本磚) σ 軌道到達 Target ← (sigmaRL3_iterate_exactB M58)。

**交付**:
- 帶橋 bitStepTape/encPair/bitStep_commute/bitStep_iterate(BitTM.step 在 Tape-編碼組態=Tape-層步的
  編碼、M25 tapeStep_nth 抬 N 步)+gStepRL3_iterate_tape。
- ★reach_at_k(核心 iff)★: (σ^[k](gEncB(bitEnc v T))).1=bitVecToNat(vhalt) ↔ ((BitTM.step)^[k]
  (encPair(v,T))).1=vhalt(M58 iterate+兩 Tape iterate+Nat.cast_inj+bitVecToNat 單射)。
- wfCfgB_bitEnc/hQ_bitEnc/hW_bitEnc(Bool k=2、K=8、wf 前提對 bitEnc 全免費)+offset_exists。
- ★sigmaM_reach_undecidable_of(機器參數化)★: 任意 TM0+封閉性+hcorr ⟹ ∃σ C^∞,¬ComputablePred(軌道
  可達)(reach_at_k+offset+Mtr_halts_iff+hcorr+funext/propext+halting_problem)。
- ★★sigmaRL3_reachability_undecidable(封頂、無條件)★★: 複用 M33 的 cu/enc/SU/hClosed/hinit_q/hcorr
  block(逐字)、尾接 sigmaM_reach_undecidable_of ⟹ 存在顯式 C^∞ 映射 σ:ℝ³→ℝ³ 其軌道可達性不可計算。

**★誠實★**: 是離散映射非連續流(裸 ℝ³ 自映射+iterate、不需緊空間/懸掛流;連續流版=死牆 M56)。C^∞ 非
analytic。可達性版本。玩具編碼(Bool 字母、Fin m→Bool 狀態經 bitVecToNat)。**與主線 M33 關係=不同數學
物件(映射 vs 流)、共用機器層、平行不取代、不冗餘不弱化**。60 模組。**★GPAC 線的「自有定理」達成:
據查全球首個關於顯式光滑映射動力系統不可判定性的機器驗證形式化★**。

## 2026-07-12 更新⑮：M60 — analytic 讀符牆（certified 負結果，把「analytic 語意分歧」機器化）

把 roadmap 多次口頭標記的「若嚴格 GPAC 要求 analytic 則 exact-on-lattice scaffold 離題」升為
機器背書的不可能性定理(零 sorry、標準三公理)、與 M56 平行:
- analytic_const_on_interval_imp_const(mathlib 恆等定理 AnalyticOnNhd.eqOn_of_preconnected_of_eventuallyEq
  +isPreconnected_univ):analytic+開區間常值⟹全域常值。
- ★no_analytic_two_plateau_reader(analytic 讀符牆)★:analytic 函數在兩開區間取不同常值⟹矛盾=沒有
  analytic 精確兩符號讀符器。
- ★sfloor_not_analytic★:sfloor(k≥2)非 AnalyticOnNhd(plateau 0 恆 0、plateau 1 恆 1、恆等定理矛盾)
  ⟹整個 exact-plateau scaffold 內稟只能 C^∞。
- ★sround_analytic(誠實對比)★:sround(sin 基 re-rounding)是 analytic⟹障礙精確定位在「精確讀符」、
  非整個 scaffold。

★誠實★:證的是「exact-plateau 技術對 analytic 不可能」、非「analytic 計算不可能」。反之
Bournez–Graça–Pouly(JACM 2017)紙上已證 analytic 多項式 ODE 可模擬 TM(近似+error-correction、另一套架構);
sround_analytic 正指出 error-corrector analytic 可得。真正 analytic GPAC 正面路線=近似讀+sround 校正、
paper-level、mathlib 未形式化、本磚不做。不影響 M33/M59(皆 C^∞、本不宣稱 analytic)。61 模組。
