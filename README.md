...
> **Status:** 70 modules · **0 `sorry`** · axioms limited to `propext`, `Classical.choice`, `Quot.sound` (the mathlib standard three) · Lean `v4.32.0-rc1` + pinned mathlib.
>
...
lake build             # builds all 70 modules; success ⇒ every theorem holds
```
...
- `M66` — second instance: **Goldbach**, in the dual shape. `GoldbachFailsFrom n := ∃ m ≥ n, GoldbachCounterexample m` is r.e. (bounded primality checks via `primrecPred_nat_prime`, unbounded search via `Partrec.rfind`); through the gate, `goldbachConjecture_iff_never_reach`: the Goldbach conjecture holds iff the orbit of `goldbachSigma` from the single point `goldbachBase 0` *never* enters the fixed `goldbachTarget`. Collatz is "every start reaches", Goldbach is "one start never reaches" (the confinement shape of `M63`). Again a reformulation, not progress; the header notes that, unlike Collatz, `GoldbachFailsFrom` may well be trivially decidable (identically false if the conjecture is true) — the content is that nobody knows which.
