...
> **Status:** 68 modules · **0 `sorry`** · axioms limited to `propext`, `Classical.choice`, `Quot.sound` (the mathlib standard three) · Lean `v4.32.0-rc1` + pinned mathlib.
>
...
lake build             # builds all 68 modules; success ⇒ every theorem holds
```
...
- `M64` — an honest "prime computer", built at the owner's request after an audit recommended against it (`docs/PRIME_COMPUTER_AUDIT_2026-09-20.md`): one fixed `BitTM`, one fixed start word, one fixed halting word, input `n` written only on the tape, and `prime_computer_uniform` / `primeSigma_reach_iff`: the `sigmaM` orbit from `primeBase n` reaches the fixed `primeTarget` iff `n` is prime. This is a *characterization of a decidable predicate* — zero computability content, one-sided (composites never reach the target; no factor is produced), and not executable — see the module's honest-scope block. Its one independently useful by-product is `primrecPred_nat_prime : PrimrecPred Nat.Prime`, which mathlib lacks.
