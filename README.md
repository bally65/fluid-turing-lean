...
- Periodic-NS companion status — `contact_geometry_lean` is verified through C247: canonical LF zero-final time tests, complete algebraic tensor consumers, faithful fixed/global kernel quotients, finite-support all-window DirectSum/image factorization, compact-support tail vanishing, nested-window zero-extension integration, and genuine late-window stability of the velocity/viscous consumer slot. Contact checkpoint commits: `612e758` / `92e2a97`. The product and complete three-slot consumers are not yet stabilized; this is still not a completed physical-test space, a Leray–Hopf package, or a Clay result.
