...
| M3 康托爾編碼 | `M3_Encoding.lean` | `Encodable Γ ↪ ℝ` 單射 + 轉移共軛；**v0.2 真康托爾集升級**：`cantorEncode : (ℕ→Bool) → ℝ` 三進位編碼，單射（首異位論證）、連續（M-判別法）、閉嵌入、shift 動力學在康托爾集上的**連續**共軛 | 0 |
...
| M6 主定理 | `M6_EulerTuring.lean` | `Simulates` 謂詞；主定理（Euler-only，paper-blocked）；**v0.2 下半層主定理（全證）**：`suspension_flow_simulates` — 緊空間上任何同胚都被緊空間上的連續 ℝ-流經單射編碼模擬；實例 `fullShift_suspension_simulates`（雙邊 full shift） | 1 ⛔ |

## sorry 帳本(v0.2：共 2，全部 paper-blocked，零可攻)
...
Axiom 檢查（`#print axioms`，2026-07-04 v0.2）：30 條已證定理僅依賴
`[propext, Classical.choice, Quot.sound]`；上表兩條額外含 `sorryAx`；**零自訂 axiom**。
特別地 `suspension_flow_simulates` / `fullShift_suspension_simulates` **不依賴任何 sorry**。
...
```
TM/generalized shift ─(M3 康托爾編碼，已證)→ 緊空間上的同胚
...
- M3 的 `encodeNat`（`3⁻ⁿ`）像集每點孤立，其上的連續性不具內容 —— v0.2 已補
  真康托爾集版本（`cantorEncode`，無孤立點，連續性是真命題）。剩餘後續工作：
  把 TM 的**局部規則結構**（generalized shift 的分塊定義）接到 `fullShift` 上。
