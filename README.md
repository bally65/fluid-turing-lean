...
- Periodic-NS companion status — `contact_geometry_lean` is verified through C259: all velocity, product-gradient, initial, and complete three-slot consumers are genuinely stable across sufficiently late windows; the canonical window-free global-time/spatial bilinear map and algebraic tensor lift have an exact realizable image, a faithful kernel quotient, and selected residual factorizations; and the global stabilized image is proved to lie in the older finite-support all-window image. Contact code/receipt commits: `8e89a6c` / `019842c`; 256 modules, 3424 source declarations, 5254 audited declarations, and 256/256 consistency checks pass. The final image comparison is only one-sided, both tensor carriers remain algebraic/uncompleted, and this is still not a completed physical-test space, a Leray–Hopf package, or a Clay result.
