...
theorem Mtr_track {Λ : Type*} [Inhabited Λ] (M : Turing.TM0.Machine Bool Λ) (S : Finset Λ)
    ...
    (c0 : Turing.TM0.Cfg Bool Λ) (hc0 : c0.q ∈ S) (k : ℕ) :
  ...
  | succ k ih =>
    ...
    · -- IH：M_tr 第 k 步 = encTM0 ck
      ...
      | some p =>
        ...
        have hq' : q' ∈ S := Finset.mem_coe.mp (hClosed hM (Finset.mem_coe.mpr hckS))
...
theorem Mtr_halts_iff {Λ : Type*} [Inhabited Λ] (M : Turing.TM0.Machine Bool Λ) (S : Finset Λ)
    (hClosed : ∀ {q : Λ} {a : Bool} {q' : Λ} {s : Turing.TM0.Stmt Bool},
        (q', s) ∈ M q a → q ∈ (↑S : Set Λ) → q' ∈ (↑S : Set Λ))
    (c0 : Turing.TM0.Cfg Bool Λ) (hc0 : c0.q ∈ S) :
  ...
  · rintro ⟨k, hk⟩
    ...
    · exact h
    · exfalso
      obtain ⟨hmtr, hckS⟩ := (Mtr_track M S hClosed c0 hc0 (k + 1)).1 ck h
      rw [hmtr] at hk
      exact encTM0_fst_ne_encHalt M S hckS hk
  · rintro ⟨k, hk⟩
    exact ⟨k, (Mtr_track M S hClosed c0 hc0 (k + 1)).2 hk⟩
