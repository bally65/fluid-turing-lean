...
- `cleanStep`（flow-window + `sround`）+ **`cleanStep_contract`**（一週期誤差 `× ρe^{−C}`）；
- **`cleanTrack_bounded`（★A HEADLINE★）**：移動整數目標軌道，tube 半徑 `r` 自洽 ⟹ `∀ n, |y n − m n| ≤ r`
...
/-- **★A 單步收縮★**：`|y−k|≤δ≤1/4`、`0≤C` ⟹ 一個清洗週期把誤差 `× ρe^{−C}`
（`ρ=1−cos2πδ`）：`|cleanStep k y C − k| ≤ ρ·e^{−C}·|y−k|`。 -/
theorem cleanStep_contract (k : ℤ) {δ C : ℝ} (hδ0 : 0 < δ) (hδ : δ ≤ 1 / 4) (hC : 0 ≤ C)
    {y : ℝ} (hy : |y - (k : ℝ)| ≤ δ) :
    |cleanStep (k : ℝ) y C - (k : ℝ)|
      ≤ (1 - Real.cos (2 * π * δ)) * Real.exp (-C) * |y - (k : ℝ)| := by
  have he0 : (0 : ℝ) ≤ Real.exp (-C) := (Real.exp_pos _).le
  have he1 : Real.exp (-C) ≤ 1 := by rw [← Real.exp_zero]; exact Real.exp_le_exp.2 (by linarith)
  set yf : ℝ := (k : ℝ) + (y - (k : ℝ)) * Real.exp (-C) with hyfdef
  have hyfk : yf - (k : ℝ) = (y - (k : ℝ)) * Real.exp (-C) := by rw [hyfdef]; ring
  have hyfabs : |yf - (k : ℝ)| = |y - (k : ℝ)| * Real.exp (-C) := by
    rw [hyfk, abs_mul, abs_of_nonneg he0]
  have hbasin : |yf - (k : ℝ)| ≤ δ := by
    rw [hyfabs]
    calc |y - (k : ℝ)| * Real.exp (-C)
        ≤ |y - (k : ℝ)| * 1 := mul_le_mul_of_nonneg_left he1 (abs_nonneg _)
      _ = |y - (k : ℝ)| := mul_one _
      _ ≤ δ := hy
  ...
  calc |sround yf - (k : ℝ)|
    ...
    _ = (1 - Real.cos (2 * π * δ)) * (|y - (k : ℝ)| * Real.exp (-C)) := by rw [hyfabs]
    _ = (1 - Real.cos (2 * π * δ)) * Real.exp (-C) * |y - (k : ℝ)| := by ring

/-- **★A HEADLINE：sround 有界追蹤（Wall B 連續版、誤差不累積）★**：整數目標軌道 `m:ℕ→ℤ`、
清洗軌道 `y:ℕ→ℝ`（每步 = flow-window 拉向 `m(n+1)` 後 `sround`）；tube 半徑 `r` 自洽
（跳幅界 `D`、`r+D≤δ`、`ρe^{−C}(r+D)≤r`）+ 起點入管 ⟹ **`∀ n, |y n − m n| ≤ r`**
（誤差對所有 N 有界、不累積——`sround` 每步清 `e^{−C}` 殘差）。 -/
theorem cleanTrack_bounded {y : ℕ → ℝ} {m : ℕ → ℤ} {r D δ C : ℝ}
    (hδ0 : 0 < δ) (hδ : δ ≤ 1 / 4) (hC : 0 ≤ C)
  ...
  | succ p ih =>
    ...
    have hstepδ : |y p - (m (p + 1) : ℝ)| ≤ δ := le_trans hstep hcons1
    have hcontr := cleanStep_contract (m (p + 1)) hδ0 hδ hC hstepδ
    rw [hrec p]
